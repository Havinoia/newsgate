...
async function fixGameNews() {
  console.log("Cleaning up old game news...");
  ...
  const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&sortBy=publishedAt&language=en&apiKey=${process.env.NEWSAPI_KEY}&pageSize=12`;

  console.log("Fetching fresh broad gaming news...");
  ...
  if (data.articles && data.articles.length > 0) {
    ...
    console.log(`Successfully ingested ${articles.length} broad gaming articles.`);
  } else {
